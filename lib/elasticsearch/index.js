...
const { get, isObject, isEmpty, merge, isNumber, isArray } = require('lodash');
...
class ElasticSearch {
  ...
  async _getAll({ index, where, offset, limit, order, unlimited }, opts = {}) {
    try {
      ...
      console.log(`[ESClient] getAll payload: ${JSON.stringify({ index, from, size, body, ...opts })}`);

      const data = await client.search({ index, from, size, body, ...opts });

      return get(data, 'hits.hits', []);
    } catch (err) {
      console.log(`[ESClient] getAll error on ${index}: ${err}`);
      throw err;
    }    
  }
